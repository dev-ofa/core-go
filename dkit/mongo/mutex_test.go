...
func TestMongoMutex_Lock(t *testing.T) {
	db := testDatabase(t)
...
func TestMongoMutex_TryLockReentrantAndExpired(t *testing.T) {
	db := testDatabase(t)
	collection := testPrefix(t) + "_mutex"
	dropCollections(t, db, collection)

	impl := NewMutexImpl(db.Collection(collection), 100*time.Millisecond)
	if err := impl.Init(); err != nil {
		t.Fatalf("init mutex impl: %v", err)
	}
